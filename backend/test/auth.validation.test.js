...
import { newPassword, email, bodyObject, otp } from '../src/modules/auth/auth.validation.js';

const env = { PGHOST: 'localhost', PGDATABASE: 'test', PGUSER: 'test', PGPASSWORD: 'local-test-only',
  JWT_SECRET: 'test-secret-'.repeat(4), INITIAL_USER_PASSWORD: 'Testpassword123!' };
test('new passwords enforce the documented policy and bcrypt byte boundary', () => {
  ...
  assert.equal(otp('012345'), '012345');
  for (const value of ['12345', '1234567', '12a456', 123456]) assert.throws(() => otp(value), { status: 400 });
});
test('auth config rejects insecure production and invalid OTP settings', () => {
  ...
  assert.throws(() => readConfig({ ...env, PASSWORD_RESET_OTP_TTL_SECONDS: '0' }));
  assert.throws(() => readConfig({ ...env, PASSWORD_RESET_OTP_MAX_ATTEMPTS: '11' }));
  assert.throws(() => readConfig({ ...env, ACCESS_TOKEN_TTL_SECONDS: '0' }));
  const config = readConfig({ ...env, NODE_ENV: 'production', CORS_ORIGIN: 'https://eris.example' });
